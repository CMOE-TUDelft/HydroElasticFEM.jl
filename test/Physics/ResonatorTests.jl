using Test
import HydroElasticFEM.Geometry as G
import HydroElasticFEM.Physics as P

@testset "ResonatorSingle struct" begin
  resn = P.ResonatorSingle(M=1e3, K=5.9e3, C=0.0)
  @test resn.M == 1e3
  @test resn.ωn1 ≈ sqrt(5.9e3 / 1e3)

  # Defaults: C and derived ωn1
  resn_def = P.ResonatorSingle(M=1e3, K=5.9e3)
  @test resn_def.C == 0.0
  @test resn_def.ωn1 ≈ sqrt(5.9e3 / 1e3)
end

@testset "resonator_array" begin
  arr = P.resonator_array(3, 100.0, 500.0, 10.0; delta_domain_symbols=[:δ1, :δ2, :δ3])
  @test length(arr) == 3
  @test arr[1] isa P.ResonatorSingle
  @test arr[2].M == 100.0
  @test arr[3].ωn1 ≈ sqrt(500.0 / 100.0)

  # Vector variant
  arr2 = P.resonator_array(2, [100.0, 200.0], [500.0, 600.0], [10.0, 20.0]; delta_domain_symbols=[:δ1, :δ2])
  @test arr2[1].M == 100.0
  @test arr2[2].M == 200.0

  # N == 0 is disallowed
  @test_throws ArgumentError P.resonator_array(0, 100.0, 500.0, 10.0)
  @test_throws ArgumentError P.resonator_array(0, Float64[], Float64[], Float64[])
  @test_throws ArgumentError P.resonator_array(2, [100.0], [500.0, 600.0], [10.0, 20.0])
end

@testset "has_*_form traits for empty resonator array" begin
  empty_resn = P.ResonatorArray(P.ResonatorSingle[])
  @test !P.has_mass_form(empty_resn)
  @test !P.has_damping_form(empty_resn)
  @test !P.has_stiffness_form(empty_resn)
  @test !P.has_rhs_form(empty_resn)
end
