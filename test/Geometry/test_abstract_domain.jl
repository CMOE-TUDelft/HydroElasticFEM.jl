...
# ─────────────────────────────────────────────────────────────
# AbstractDomain interface — TankDomain2D
# ─────────────────────────────────────────────────────────────
...
@testset "AbstractDomain — TankDomain2D subtype" begin
  @test G.TankDomain2D <: G.AbstractDomain
  tank = G.TankDomain2D(L=4.0, H=1.0, nx=20, ny=2)
  @test tank isa G.AbstractDomain
end

@testset "ambient_dimension / manifold_dimension — TankDomain2D" begin
  tank = G.TankDomain2D()
  @test G.ambient_dimension(tank) == 2
  @test G.manifold_dimension(tank) == 2
end

@testset "boundary_tags — TankDomain2D" begin
  tank  = G.TankDomain2D()
...
@testset "get_boundary — TankDomain2D non-structure tags" begin
  tank = G.TankDomain2D(L=4.0, H=1.0, nx=20, ny=4)
...
@testset "get_boundary — TankDomain2D fluid (interior)" begin
  tank = G.TankDomain2D(L=4.0, H=1.0, nx=20, ny=4)
...
@testset "get_boundary — TankDomain2D structure (empty when no structures)" begin
  tank  = G.TankDomain2D(L=4.0, H=1.0, nx=20, ny=4)
...
@testset "get_boundary — TankDomain2D invalid tag raises error" begin
  tank = G.TankDomain2D()
  @test_throws ErrorException G.get_boundary(tank, "nonexistent_tag")
end

@testset "triangulation — TankDomain2D" begin
  tank = G.TankDomain2D(L=4.0, H=1.0, nx=10, ny=2)
