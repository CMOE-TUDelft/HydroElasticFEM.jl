...
name::String = "data/sims_202309/run/mono_freq_free"
order::Int = 2
...
ω = 1.6#3.45#2.0#2.4
η₀ = 0.10
k0 = dispersionRelAng(H0, ω)
k1 = dispersionRelAng(H1, ω)
λ0 = 2*π/k0
λ1 = 2*π/k1
T = 2π/ω
ηᵢₙ(x) = η₀*exp(im*k0*x[1])
ϕᵢₙ(x) = -im*(η₀*ω/k0)*(cosh(k0*(H0 + x[2])) / sinh(k0*H0))*exp(im*k0*x[1])
vxᵢₙ(x) = (η₀*ω)*(cosh(k0*(H0 + x[2])) / sinh(k0*H0))*exp(im*k0*x[1])
vzfsᵢₙ(x) = -im*ω*η₀*exp(im*k0*x[1]) #???
@show λ0, λ1
@show T
@show η₀
@show H0
...
# Mesh
# function f_xy(x, y, r, n, H0, H1; dbgmsg = false)
...
#   end
# end
# map(x) = VectorValue( x[1], f_xy(x[1], x[2], mesh_ry, ny, H0, H1) )

# Mesh
function f_xy(x, y, r, n, H0, H1, H2; dbgmsg = false)
  # Mesh along depth as a GP
  # Depth is 0 to -H0    
  if(r ≈ 1.0)
    return y  
    ...
    elseif( 0.4 <= Δx <= 0.6)
      HRef = H2
    elseif( Δx < 0.4 )
      HRef = H0 + (H2-H0)/0.4*Δx
    elseif( Δx > 0.6 )
      HRef = H2 + (H1-H2)/0.4*(Δx - 0.6)
    end
...
map(x) = VectorValue( x[1], f_xy(x[1], x[2], mesh_ry, ny, 
  H0, H1, 4.0) )
...
ηa(x) = η₀*(cos(x[1]*k0) + im*sin(x[1]*k0))
...
kh0 = k0*H0
wave_n0 = 0.5*(1 + 2*kh0/sinh(2*kh0))
kh1 = k1*H1
wave_n1 = 0.5*(1 + 2*kh1/sinh(2*kh1))
Pin = (0.5*ρw*g*η₀*η₀)*(ω/k0)*wave_n0
Prf = (0.5*ρw*g*ηrf*ηrf)*(ω/k0)*wave_n0
Ptr = (0.5*ρw*g*ηtr*ηtr)*(ω/k1)*wave_n1
